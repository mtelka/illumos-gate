 ...
 * interfaces. These are imported from the OFED header rdma_cm.h. Oracle
 * elects to have and use the contents of rdma_cm.h under and governed
...
#ifndef _SYS_IB_CLIENTS_OF_RDMA_RDMA_CM_H
#define	_SYS_IB_CLIENTS_OF_RDMA_RDMA_CM_H
...
struct rdma_addr {
	...
	uint8_t		src_pad[sizeof (struct sockaddr_in6) -
				sizeof (struct sockaddr)];
	struct sockaddr dst_addr;
	uint8_t		dst_pad[sizeof (struct sockaddr_in6) -
				sizeof (struct sockaddr)];
 ...
 *
 * rdma_map_id2qphdl -
 *	This is used to map RDMA-CM ID to IBTF or ROF client handle and
 *	the QP handle. This is called by the passive side just after
 *	rdma_create_id() and on the active side just before rdma_accept()
 *	in the CONNECTION_REQUEST event handler.
 */
...
#endif
#endif /* _SYS_IB_CLIENTS_OF_RDMA_RDMA_CM_H */
