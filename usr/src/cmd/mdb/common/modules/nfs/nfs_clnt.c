...
nfs_servinfo_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	mdb_printf("address = ");
	if (mdb_vread(&knconf, sizeof (knconf), (uintptr_t)si.sv_knconf) == -1) {
		mdb_printf("?\?/?\?/??");
	} else {
		...
		if (mdb_readstr(knc_str, sizeof (knc_str), (uintptr_t)knconf.knc_protofmly) == -1)
			mdb_printf("/??");
		else
			mdb_printf("/%s", knc_str);

		if (mdb_readstr(knc_str, sizeof (knc_str), (uintptr_t)knconf.knc_proto) == -1)
			mdb_printf("/??");
		else
			mdb_printf("/%s", knc_str);
	}
...
static const mdb_bitmask_t nfs_mi4_flags[] = {
	...
	{"MI4_EPHEMERAL_RECURSED", MI4_EPHEMERAL_RECURSED, MI4_EPHEMERAL_RECURSED},
	...
	{"MI4_REMOVE_ON_LAST_CLOSE", MI4_REMOVE_ON_LAST_CLOSE, MI4_REMOVE_ON_LAST_CLOSE},
...
mimsg_print_event(const nfs4_debug_msg_t *msg)
{
	...
	case RE_FILE_DIFF:
		...
		break;
	case RE_LOST_STATE:
		/* TODO: if char1 is null you should use ::nfs4_fname for re_rp1 */
...
mimsg_print_fact(const nfs4_debug_msg_t *msg)
{
	...
	if (mdb_readstr(msg_srv, sizeof (msg_srv), (uintptr_t)msg->msg_srv) == -1)
		strcpy(msg_srv, "??");
	...
	case RF_DELMAP_CB_ERR:
		if (mdb_readstr(file, sizeof (file), (uintptr_t)fp->rf_char1) == -1)
			strcpy(file, "??");
	...
	if (opt_s) {
		...
		case RM_EVENT:
			...
			for (i = 0; nfs4_event_type_tbl[i].str != NULL; i++)
				if (nfs4_event_type_tbl[i].et == msg.rmsg_u.msg_event.re_type) {
		...
		case RM_FACT:
			...
			for (i = 0; nfs4_fact_type_tbl[i].str != NULL; i++)
				if (nfs4_fact_type_tbl[i].ft == msg.rmsg_u.msg_fact.rf_type) {
...
print_nfs4_fname(uintptr_t addr)
{
	...
	*p = '\0';
	while (addr != NULL) {
		...
		if ((addr = (uintptr_t)fn.fn_parent) != NULL)
			*--p = '/';
	}
	mdb_printf("%s", p);
}
