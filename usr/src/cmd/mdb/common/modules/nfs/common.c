...
static int
zsd_finder(uintptr_t addr, const void *data, void *cb_data)
{
 ...
 */
int
zoned_zsd_find_by_key(uintptr_t zoneaddr, zone_key_t key, uintptr_t *result)
{
	...
	if (mdb_pwalk("zsd", zsd_finder, &cbd, zoneaddr) != 0) {
		mdb_warn("failed to walk zsd\n");
		return (DCMD_ERR);
	}

	if (cbd.found == FALSE) {
		mdb_printf("no ZSD entry found");
		return (DCMD_ERR);
	}
