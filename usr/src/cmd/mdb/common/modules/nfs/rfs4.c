...
rfs4_db_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if ((flags & DCMD_ADDRSPEC) == 0) {
		if (mdb_readsym(&addr, sizeof (addr),
		    "rfs4_server_state") == -1) {
			mdb_warn("can't readsym %s", "rfs4_server_state");
			return (DCMD_ERR);
		}
	}

	if (mdb_vread(&rfs4_db, sizeof (rfs4_db), addr) == -1) {
		mdb_warn("can't read rfs4_database_t");
		return (DCMD_ERR);
	}

	mdb_printf("rfs4_database = %p\n", addr);
	mdb_printf("debug_flags   = %08x shutdown_count = %d  tables = %p\n",
	    rfs4_db.db_debug_flags, rfs4_db.db_shutdown_count,
	    rfs4_db.db_tables);
...
rfs4_oo_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_ADDRSPEC)
		return (rfs4_print_oo(addr, NULL));
...
rfs4_print_deleg_cb(uintptr_t addr, const void *data, void *cb_data)
{
	...
	return (rfs4_print_deleg(addr, *(uint_t *)cb_data, NULL) == DCMD_OK
	    ? WALK_NEXT : WALK_ERR);
}
...
rfs4_deleg_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_ADDRSPEC)
		return (rfs4_print_deleg(addr, opt_v, NULL));
...
rfs4_lo_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	...
	if (flags & DCMD_ADDRSPEC)
		return (rfs4_print_lo(addr, NULL));
