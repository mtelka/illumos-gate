...
int
nfs4_idmap_info_dcmd(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
...
nfs4_idmap_walk_init(mdb_walk_state_t *wsp)
{
	...
	/* key = nfsidmap_zone_key */
	if (mdb_readsym(&key, sizeof (zone_key_t), "nfsidmap_zone_key") == -1) {
		mdb_warn("failed to get %s", "nfsidmap_zone_key");
		return (WALK_ERR);
	}

	/* wsp->walk_addr = zone_getspecific(key, wsp->walk_addr) */
	if (zoned_zsd_find_by_key(wsp->walk_addr, key, &wsp->walk_addr)
	    != DCMD_OK) {
	...
	/*
	 * table =
	 *     ((struct nfsidmap_globals *)wsp->walk_addr)->CACHE_INFO.table
	 */
